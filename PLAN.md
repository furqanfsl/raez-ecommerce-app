...
- **D3.5** — **Tell Claude:** *"Create `com.raez.util.MigratePasswords` with main method:
  ...
  4. Idempotent.
    se `System.getProperty('user.home')` for the backup path (Windows-safe)."*
...
> "I'm working through PLAN.md (v2). I'm on step **D{2.1}**. Read PLAN.md for context, then execute that step. If anything is ambiguous, ask before changing files."
