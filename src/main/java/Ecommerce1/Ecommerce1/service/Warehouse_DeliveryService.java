...
public class Warehouse_DeliveryService {
    ...
    public ConfirmResult confirmDelivery(int deliveryId, int numItems, int warehouseId) {
        ...
        String findProduct    = "SELECT productID FROM warehouse_inventory WHERE warehouseID = ? LIMIT 1";
        // CHANGED: InventoryRecord → warehouse_inventory
        String updateStock    = "UPDATE warehouse_inventory SET quantityOnHand = quantityOnHand + ? " +
        ...
        try (Connection conn = Warehouse_DatabaseConnection.getConnection()) {
            ...
            conn.commit();
            return new ConfirmResult(true, warehouseId, productId, numItems);
